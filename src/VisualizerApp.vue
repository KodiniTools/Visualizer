              ...
              <div class="preview-modal-content">
                <div class="preview-modal-image-container">
                  <img
                    ...
                    :src="previewImageData.imageObject.src"
                    alt="Preview"
                  />
                </div>
                <div class="preview-modal-info">
                  ...
                  <div class="preview-info-grid" v-if="previewImageData">
                    <div class="preview-info-item">
                      ...
                      <span class="preview-info-value"
                        >{{ previewImageIndex + 1 }} / {{ canvasImages.length }}</span
                      >
                    </div>
                    <div class="preview-info-item">
                      ...
                      <span class="preview-info-value"
                        ...
                        {{ previewImageData.imageObject.naturalHeight }} px</span
                      >
                    </div>
                    <div class="preview-info-item">
                      ...
                      <span class="preview-info-value"
                        ...
                        {{ Math.round(previewImageData.relY * 100) }}%</span
                      >
                    </div>
                    <div class="preview-info-item">
                      ...
                      <span class="preview-info-value"
                        ...
                        {{ Math.round(previewImageData.relHeight * 100) }}%</span
                      >
                    </div>
                  </div>
                  <!-- Ersetzen-Buttons -->
                  <div class="preview-modal-actions">
                    ...
                    <div class="replace-buttons-row">
                      ...
                      />
                      <button
                        class="btn-replace-canvas-image"
                        @click="replaceCanvasImageInput?.click()"
                      >
                        📁 {{ t('app.uploadImage') }}
                      </button>
                      <button
                        class="btn-replace-canvas-image btn-gallery"
                        @click="openReplaceGallery"
                      >
                        🖼️ {{ t('app.fromGallery') }}
                      </button>
                    </div>
                  ...
                  <div v-if="pendingReplaceImageSrc" class="pending-replace-preview">
                    ...
                    <div class="pending-replace-image-container">
                      <img
                        :src="pendingReplaceImageSrc"
                        alt="Preview"
                        class="pending-replace-image"
                      />
                    </div>
                ...
                <div v-else class="replace-gallery-grid">
                  ...
                  >
                    <img
                      ...
                      loading="lazy"
                    />
                    <span v-if="image.name" class="replace-gallery-item-name">{{
                      image.name
                    }}</span>
                  </div>
                </div>
...
function applyAudioData(data) {
  ...
  if (data.smoothBass !== undefined) {
    ...
    window.audioAnalysisData.smoothTreble = Math.floor(
      window.audioAnalysisData.smoothTreble * (1 - trebleSmoothFactor) +
        data.treble * trebleSmoothFactor,
    )
...
const stopVisualizerLoop = () => {
  console.log(
    `[App] stopVisualizerLoop (aktiv: ${isVisualizerActive}, ID: ${visualizerAnimationId})`,
  )
...
async function setupMicrophoneSource() {
  try {
    ...
    console.log(
      '[App] Created AudioContext for microphone, sample rate:',
      microphoneAudioContext.sampleRate,
    )
    ...
    console.log(
      '[App] Microphone audio graph: source -> gain -> silent -> destination (for Chrome bug)',
    )
    console.log('[App] Microphone AudioContext state:', microphoneAudioContext.state)
    console.log(
      '[App] Microphone Analyser fftSize:',
...
async function toggleRecordingMicrophone(enable) {
  ...
  if (enable) {
    ...
    console.log(
      '[App] Gain-Werte: Player:',
      recordingGain.gain.value,
      'Mic:',
      micRecordingGain.gain.value,
    )
  } else {
...
async function createCombinedAudioStream() {
  ...
  if (!micConnected) {
    console.warn(
      '[App] Mic-Recording-Stream konnte nicht verbunden werden - Live-Umschaltung auf Mic nicht möglich',
    )
  } else {
  ...
  tracks.forEach((track, i) => {
    console.log(
      `[App] Recording Track ${i}: ${track.label}, enabled: ${track.enabled}, readyState: ${track.readyState}`,
    )
  })
...
window.debugRecordingAudio = function () {
  ...
  console.log(
    'Audio-Graph: [Player→recordingGain] + [Mic→micRecordingGain] → recordingMixer → recordingDest',
  )
  ...
  if (micRecordingStream) {
    ...
    recMicTracks.forEach((t, i) => {
      console.log(
        `  Rec Mic Track ${i}: ${t.label}, enabled: ${t.enabled}, readyState: ${t.readyState}`,
      )
    })
  } else {
  ...
  if (audioSourceStore.microphoneStream) {
    ...
    visMicTracks.forEach((t, i) => {
      console.log(
        `  Vis Mic Track ${i}: ${t.label}, enabled: ${t.enabled}, readyState: ${t.readyState}`,
      )
    })
  }
...
window.takeCanvasScreenshot = async function (mimeType = 'image/png', quality = 0.9) {
  ...
  if (canvasManagerInstance.value?.workspacePreset) {
    const preset =
      canvasManagerInstance.value.socialMediaPresets[canvasManagerInstance.value.workspacePreset]
    if (preset) {
  ...
  watch(
    ...
    (isRecording) => {
      if (isRecording) {
        ...
        console.log(
          '[App] Frischer Recording Canvas erstellt:',
          recordingCanvas.width,
          'x',
          recordingCanvas.height,
        )
