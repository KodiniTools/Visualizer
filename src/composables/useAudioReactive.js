...
export function useAudioReactive(fotoManagerRef, currentActiveImage) {
  ...
  function loadAudioReactiveSettings(imageData) {
    ...
    if (!imageData?.fotoSettings?.audioReactive) {
      ...
      if (audioReactiveSmoothingValueRef.value)
        audioReactiveSmoothingValueRef.value.textContent = '50%'
      ...
      if (audioReactiveBeatBoostValueRef.value)
        audioReactiveBeatBoostValueRef.value.textContent = 'Aus'
    ...
    if (audioReactiveSmoothingValueRef.value)
      audioReactiveSmoothingValueRef.value.textContent = (ar.smoothing || 50) + '%'
    ...
    if (audioReactiveBeatBoostRef.value) {
      ...
      audioReactiveBeatBoostRef.value.value = beatBoost
      if (audioReactiveBeatBoostValueRef.value) {
        audioReactiveBeatBoostValueRef.value.textContent =
          beatBoost <= 1.0 ? 'Aus' : `${Math.round((beatBoost - 1) * 100)}%`
      }
    }
    if (audioReactivePhaseRef.value) {
  ...
  function startAudioLevelIndicator() {
    ...
    function updateLevel() {
      ...
      switch (source) {
        case 'bass':
          level = audioData.smoothBass
          break
        case 'mid':
          level = audioData.smoothMid
          break
        case 'treble':
          level = audioData.smoothTreble
          break
        case 'volume':
          level = audioData.smoothVolume
          break
      }
      ...
      if (percent > 70) {
        audioLevelBarRef.value.style.background =
