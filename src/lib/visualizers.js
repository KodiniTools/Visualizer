 ...
 */
function getFrequencyBasedSmoothing(
  barIndex,
  totalBars,
...
export const Visualizers = {
  ...
  matrixRain: {
    ...
    init(width, height) {
      ...
      visualizerState.matrixRain = {
        ...
        chars: Array.from({ length: columns }, () =>
          Array.from({ length: 30 }, () => String.fromCharCode(0x30a0 + Math.random() * 96)),
        ),
      }
    },
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      if (
        !visualizerState.matrixRain ||
        visualizerState.matrixRain.columns !== Math.floor(width / 16)
      ) {
    ...
    draw(ctx, dataArray, bufferLength, w, h, color, intensity = 1.0) {
      ...
      for (let i = 0; i < numBars / 2; i++) {
        ...
        visualizerState.smoothedMirroredBars[i] = applySmoothValue(
          visualizerState.smoothedMirroredBars[i] || 0,
          targetHeight,
          smoothingFactor,
        )
  ...
  radialBars: {
    ...
    draw(ctx, dataArray, bufferLength, w, h, color, intensity = 1.0) {
      ...
      if (
        !visualizerState.smoothedRadialBars ||
        visualizerState.smoothedRadialBars.length !== numBars
      ) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        for (let i = 0; i < numBars; i++) {
          ...
          visualizerState.smoothedRadialBars[i] =
            visualizerState.smoothedRadialBars[i] * 0.6 + targetLength * 0.4
  ...
  vibratingCubes: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      for (let i = 0; i < numCubes; i++) {
        ...
        const dynamicGain = calculateDynamicGain(i, numCubes, {
          bassGain: 1.0,
          lowMidGain: 1.3,
          highMidGain: 1.8,
          highGain: 2.8,
        ...
        withSafeCanvasState(ctx, () => {
          ...
          if (normalizedPos > 0.6) {
            ctx.translate(
              (Math.random() - 0.5) * amplitude * 5,
              (Math.random() - 0.5) * amplitude * 5,
            )
          }
          ctx.beginPath()
          ctx.moveTo(0, 0)
          ctx.lineTo(-cubeWidth / 2, -cubeWidth / 4)
          ctx.lineTo(-cubeWidth / 2, -cubeWidth / 4 - cubeHeight)
          ctx.lineTo(0, -cubeHeight)
          ctx.closePath()
          ...
          ctx.fill()
          ctx.beginPath()
          ctx.moveTo(0, -cubeHeight)
          ctx.lineTo(cubeWidth / 2, -cubeWidth / 4 - cubeHeight)
          ctx.lineTo(0, -cubeWidth / 2 - cubeHeight)
          ctx.lineTo(-cubeWidth / 2, -cubeWidth / 4 - cubeHeight)
          ctx.closePath()
  ...
  waveform: {
    ...
    draw(ctx, dataArray, bufferLength, w, h, color, intensity = 1.0) {
      ...
      const wfState =
        visualizerState._waveformState ||
        (visualizerState._waveformState = { bassPrev: 0, beatFlash: 0 })
      ...
      withSafeCanvasState(ctx, () => {
        ...
        gradient.addColorStop(
          1,
          `hsla(${(baseHsl.h + 40) % 360}, 100%, 60%, ${0.38 + totalEnergy * 0.3})`,
        )
  ...
  fluidWaves: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      if (
        !visualizerState.smoothedFluidWaves ||
        visualizerState.smoothedFluidWaves.length !== numWaves ||
        visualizerState.smoothedFluidWaves[0].length !== points + 1
      ) {
        visualizerState.smoothedFluidWaves = Array.from(
          { length: numWaves },
      ...
      const midEnergy =
        averageRange(dataArray, Math.floor(maxFreqIndex * 0.2), Math.floor(maxFreqIndex * 0.6)) /
        255
      ...
      withSafeCanvasState(ctx, () => {
        ...
        for (let wave = numWaves - 1; wave >= 0; wave--) {
          ...
          for (let i = 0; i <= points; i++) {
            ...
            const sineBase =
              Math.sin(x * 0.007 + now * (0.8 + wave * 0.15) + wave * 1.1) *
              (16 + overallEnergy * 28)
            ...
            visualizerState.smoothedFluidWaves[wave][i] =
              visualizerState.smoothedFluidWaves[wave][i] * (1 - smoothFactor) +
              targetY * smoothFactor
          }
          ...
          fillGrad.addColorStop(
            0,
            `hsla(${hue}, 100%, ${62 + frontness * 12}%, ${0.55 + bassEnergy * 0.3})`,
          )
          fillGrad.addColorStop(
            0.55,
            `hsla(${hue}, ${baseHsl.s}%, ${Math.min(68, baseHsl.l + 18)}%, ${0.38 + overallEnergy * 0.2})`,
          )
  ...
  spiralGalaxy: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      if (
        !visualizerState.smoothedSpiralGalaxy ||
        visualizerState.smoothedSpiralGalaxy.length !== totalPoints
      ) {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        for (let i = 0; i < numSegments; i++) {
          ...
          gradient.addColorStop(
            0.5,
            `hsla(${hue}, ${baseHsl.s}%, ${Math.min(80, baseHsl.l + 20)}%, 0.7)`,
          )
  ...
  rippleEffect: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      const bassEnergy = averageRange(dataArray, 0, Math.floor(maxFreqIndex * 0.3)) / 255
      const midEnergy =
        averageRange(dataArray, Math.floor(maxFreqIndex * 0.3), Math.floor(maxFreqIndex * 0.7)) /
        255
      ...
      if (bgRamp > 0.005) {
        ...
        pulseGrad.addColorStop(
          0.5,
          `hsla(${(baseHsl.h + 30) % 360}, 70%, 20%, ${bgRamp * bassEnergy * 0.1})`,
        )
  ...
  waveformHorizon: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      const midEnergy =
        averageRange(dataArray, Math.floor(maxFreqIndex * 0.15), Math.floor(maxFreqIndex * 0.5)) /
        255
  ...
  frequencyBlossoms: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        glow.addColorStop(
          0.5,
          `hsla(${(baseHsl.h + 30) % 360}, 100%, 50%, ${0.1 + bassEnergy * 0.2})`,
        )
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        for (let r = 0; r < numRays; r++) {
          ...
          const targetLength =
            baseRadius + (0.3 + amplitude * 0.7 + vibration) * (maxRadius - baseRadius)
  ...
  shardMosaic: {
    ...
    init(width, height) {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          const p1 = { x: c * cellWidth, y: r * cellHeight },
            p2 = { x: (c + 1) * cellWidth, y: r * cellHeight }
          const p3 = { x: c * cellWidth, y: (r + 1) * cellHeight },
            p4 = { x: (c + 1) * cellWidth, y: (r + 1) * cellHeight }
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      if (
        !visualizerState.shardMosaic ||
        visualizerState.shardTriangles.length === 0 ||
        visualizerState.shardMosaic.lastWidth !== width ||
        visualizerState.shardMosaic.lastHeight !== height
      ) {
        this.init(width, height)
      }
      const baseHsl = hexToHsl(color)
      ...
      withSafeCanvasState(ctx, () => {
        ...
        if (bassEnergy > 0.2) {
          ...
          glowGradient.addColorStop(
            0.5,
            `hsla(${(baseHsl.h + 30) % 360}, 100%, 50%, ${bassEnergy * 0.2})`,
          )
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      for (let wave = 0; wave < numWaves; wave++) {
        ...
        for (let i = 0; i < numPoints; i++) {
          ...
          const baseWave =
            Math.sin(x * 0.018 + now * (1.5 + wave * 0.4) + wave * 1.1) * (18 + bassEnergy * 22)
  ...
  pulsingOrbs: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      const ps =
        visualizerState._pulsingOrbsState ||
        (visualizerState._pulsingOrbsState = {
  ...
  synthWave: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      const swState =
        visualizerState._synthWaveState ||
        (visualizerState._synthWaveState = { bassPrev: 0, beatPulse: 0 })
      ...
      withSafeCanvasState(ctx, () => {
        ...
        sunGradient.addColorStop(
          0,
          `hsla(${(baseHsl.h + 30) % 360}, 100%, ${70 + swState.beatPulse * 12}%, ${0.9 + bassEnergy * 0.1})`,
        )
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      if (bassEnergy > 0.2) {
        ...
        centerGlow.addColorStop(
          0.5,
          `hsla(${(baseHsl.h + 60) % 360}, 80%, 40%, ${bassEnergy * 0.08})`,
        )
      ...
      state.particles.forEach((p, index) => {
        ...
        gradient.addColorStop(
          0.7,
          `hsla(${(hue + 30) % 360}, 80%, 40%, ${(minAlpha + amplitude * 0.2) * 0.3})`,
        )
        gradient.addColorStop(1, 'transparent')
  ...
  heartbeat: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        gradient.addColorStop(
          0,
          `hsl(${baseHsl.h}, ${baseHsl.s}%, ${Math.min(85, baseHsl.l + 20)}%)`,
        )
        gradient.addColorStop(0.6, `hsl(${baseHsl.h}, ${baseHsl.s}%, ${baseHsl.l}%)`)
        gradient.addColorStop(
          1,
          `hsl(${baseHsl.h}, ${Math.max(40, baseHsl.s - 20)}%, ${Math.max(20, baseHsl.l - 20)}%)`,
        )
        ...
        const bpm = Math.min(
          180,
          Math.max(40, Math.round(60000 / Math.max(500, timeSinceLastPulse))),
        )
      ...
      withSafeCanvasState(ctx, () => {
        for (let i = state.signals.length - 1; i >= 0; i--) {
          ...
          if (signal.progress >= 1) {
            ...
            state.neurons[signal.to].activation = Math.min(
              1,
              state.neurons[signal.to].activation + 0.5,
            )
  ...
  cellGrowth: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      if (
        overallEnergy > spawnThreshold &&
        now > state.nextSpawn &&
        state.cells.length < maxCells
      ) {
      ...
      for (let i = state.cells.length - 1; i >= 0; i--) {
        ...
        withSafeCanvasState(ctx, () => {
          ...
          cellGradient.addColorStop(
            0,
            `hsla(${cell.hue}, ${baseHsl.s}%, ${Math.min(80, baseHsl.l + 20)}%, ${alpha})`,
          )
          cellGradient.addColorStop(
            0.5,
            `hsla(${cell.hue}, ${baseHsl.s}%, ${baseHsl.l}%, ${alpha})`,
          )
          cellGradient.addColorStop(
            1,
            `hsla(${cell.hue}, ${Math.max(40, baseHsl.s - 20)}%, ${Math.max(30, baseHsl.l - 10)}%, ${alpha})`,
          )
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      state.emitters.forEach((emitter, index) => {
        ...
        const body = ctx.createRadialGradient(
          emitter.x - radius * 0.25,
          emitter.y - radius * 0.25,
          0,
          emitter.x,
          emitter.y,
          radius,
    ...
    draw(ctx, dataArray, bufferLength, w, h, color, intensity = 1.0) {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          const freqIndex =
            Math.floor((distFromCenter + waveOffset * 0.3) * maxFreqIndex * 2) % maxFreqIndex
          ...
          if (smoothedValue > 0.05) {
            ...
            const colorIndex = Math.floor(
              (distFromCenter * 3 + smoothedValue * 2) % neonColors.length,
            )
  ...
  // ============================================================================
  // ORBITING LIGHT - Kreisender Lichteffekt mit Audio-Reaktion
  // ============================================================================
  orbitingLight: {
    ...
    draw(ctx, dataArray, bufferLength, width, height, color, intensity = 1.0) {
      ...
      withSafeCanvasState(ctx, () => {
        ...
        centerGlow.addColorStop(
          0,
          `hsla(${baseHsl.h}, 100%, 60%, ${state.smoothedEnergy * 0.3 * intensity})`,
        )
        centerGlow.addColorStop(
          0.5,
          `hsla(${baseHsl.h}, 80%, 40%, ${state.smoothedEnergy * 0.15 * intensity})`,
        )
        ...
        // Lichter zeichnen
        state.lights.forEach((light, index) => {
          // Geschwindigkeit basierend auf Frequenzband
          ...
          // Orbit-Radius reagiert auf Bass (pulsiert)
          const dynamicRadius =
            light.orbitRadius * (1 + state.smoothedBass * 0.4) * (maxRadius / 200)
          ...
          // Verbindungslinien zum Zentrum bei hoher Energie
        ...
        // Zentraler Punkt pulsiert mit Bass
        const centerSize = 5 + state.smoothedBass * 20
        const centerGlowInner = ctx.createRadialGradient(
          centerX,
          centerY,
          0,
          centerX,
