...
export const fluidWaves = {
  ...
  draw(ctx, dataArray, bufferLength, w, h, color, intensity = 1.0) {
    ...
    for (let wave = 0; wave < numWaves; wave++) {
      ...
      visualizerState.smoothedFluidWaves[wave] =
        visualizerState.smoothedFluidWaves[wave] * 0.8 + amplitude * 0.2
      ...
      for (let x = 0; x <= w; x += step) {
        const y =
          baseY + Math.sin(x * 0.01 + time * (1 + wave * 0.3) + wave) * waveHeight * intensity
        ctx.lineTo(x, y)
      }
      ...
      gradient.addColorStop(
        0,
        `hsla(${hue}, ${baseHsl.s}%, ${baseHsl.l}%, ${0.4 + smoothedAmp * 0.4})`,
      )
