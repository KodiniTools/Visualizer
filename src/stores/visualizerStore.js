...
export const useVisualizerStore = defineStore('visualizer', () => {
  ...
  function removeLayer(layerId) {
    ...
    if (activeLayerId.value === layerId) {
      if (visualizerLayers.value.length > 0) {
        activeLayerId.value =
          visualizerLayers.value[Math.min(index, visualizerLayers.value.length - 1)].id
      } else {
  ...
  function fallbackToLastWorking() {
    console.warn(
      `⚠️ [VisualizerStore] Visualizer "${selectedVisualizer.value}" fehlgeschlagen - Fallback zu "${lastWorkingVisualizer.value}"`,
    )
    selectedVisualizer.value = lastWorkingVisualizer.value
  }
